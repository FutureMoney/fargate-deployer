...
import { Manifest, ManifestKind } from './types';
...
const FARGATE_SIZES: Record<number, { min: number; max: number; step: number }> = {
...
const KINDS: ManifestKind[] = ['Service', 'ScheduledTasks'];

const SCHEDULE_PATTERN = /^(cron\(.+\)|rate\(\d+\s+(minute|minutes|hour|hours|day|days)\))$/;

export function validateManifest(raw: unknown, source: string): Manifest {
  ...
  if (!KINDS.includes(m.kind)) {
    issues.add('kind', `must be one of ${KINDS.join(' | ')}${m.kind ? `, got ${JSON.stringify(m.kind)}` : ''}`,
      '`Service` for a long-running container, `ScheduledTasks` for cron jobs.');
  }

  requireString(issues, m, 'name');
  if (typeof m.name === 'string' && !/^[a-z0-9][a-z0-9-]{0,239}$/.test(m.name)) {
    issues.add('name', 'must be lowercase letters, digits and hyphens, starting with a letter or digit',
      'It becomes the ECS service name, task family and (by default) the stack name.');
  }

  if (typeof m.account !== 'string' || !/^\d{12}$/.test(m.account)) {
    issues.add('account', 'must be a 12-digit AWS account ID as a string',
      'Quote it in YAML — an unquoted number loses leading zeros.');
  }
  ...
  if (!isObject(m.network)) {
    ...
    if (!Array.isArray(n.subnets) || n.subnets.length === 0) {
      issues.add('network.subnets', 'must list at least one subnet ID',
        'Two subnets in different availability zones is the usual minimum for a load-balanced service.');
    } else {
      n.subnets.forEach((s: unknown, i: number) => {
    ...
    if (n.securityGroups !== undefined) {
      if (!Array.isArray(n.securityGroups)) {
        issues.add('network.securityGroups', 'must be a list of security group IDs',
          'Omit it entirely to have one created for you.');
      } else {
        n.securityGroups.forEach((sg: unknown, i: number) => {
  ...
  if (!isObject(m.task)) {
    ...
    if (m.task.logRetentionDays !== undefined && !VALID_LOG_RETENTION_DAYS.includes(m.task.logRetentionDays)) {
      issues.add('task.logRetentionDays', `must be one of ${VALID_LOG_RETENTION_DAYS.join(', ')}`,
        'CloudWatch only accepts these retention periods. 0 means never expire.');
    }
    ...
    if (m.task.runtimePlatform !== undefined) {
      ...
      if (arch !== undefined && arch !== 'X86_64' && arch !== 'ARM64') {
        issues.add('task.runtimePlatform.cpuArchitecture', 'must be `X86_64` or `ARM64`',
          'Use ARM64 only if your image is built for it (`docker buildx --platform linux/arm64`).');
      }
    }
  }
  ...
  if (m.kind === 'Service') {
    ...
    if (m.tasks !== undefined) {
      issues.add('tasks', 'is only valid when `kind: ScheduledTasks`',
        'A `Service` runs continuously; it has no schedule.');
    }
  } else if (m.kind === 'ScheduledTasks') {
    validateScheduledTasks(issues, m);
    for (const field of ['service', 'loadBalancer', 'autoScaling']) {
      if (m[field] !== undefined) {
        issues.add(field, 'is only valid when `kind: Service`',
          'Scheduled tasks run to completion, so they are not load balanced or auto scaled.');
      }
    }
  }
...
function validateService(issues: IssueCollector, m: Record<string, any>): void {
  ...
  if (lb !== undefined && !isObject(lb)) {
    ...
    if (!lb.listenerArn && !lb.loadBalancerArn) {
      issues.add('loadBalancer', 'needs either `listenerArn` or `loadBalancerArn`',
        'Attach to an existing listener with `listenerArn` (recommended, lets services share one ALB), ' +
    ...
    if (lb.securityGroupId !== undefined && !/^sg-[0-9a-f]{8,}$/.test(String(lb.securityGroupId))) {
      issues.add('loadBalancer.securityGroupId', 'must be a security group ID like `sg-0abc123def456789`',
        "This is the *load balancer's* security group. It is used to allow the ALB into the task security group.");
    }
    if (lb.securityGroupId === undefined && lb.manageSecurityGroupRules !== false) {
      issues.add('loadBalancer.securityGroupId', 'is required so the ALB can reach your tasks',
        'Give the load balancer\'s security group ID, or set `manageSecurityGroupRules: false` if you ' +
    ...
    if (lb.loadBalancerArn && !lb.listenerArn && protocol === 'HTTPS' && !lb.certificateArn) {
      issues.add('loadBalancer.certificateArn', 'is required to create an HTTPS listener',
        'Either give an ACM certificate ARN, set `listenerProtocol: HTTP`, or attach to an existing ' +
    ...
    if (lb.targetPort === undefined && m.task?.containerPort === undefined) {
      issues.add('task.containerPort', 'is required for a load-balanced service',
        'Set `task.containerPort` (and optionally a different `loadBalancer.targetPort`).');
    }
    ...
    if (lb.defaultAction === true && (hosts.length > 0 || paths.length > 0)) {
      issues.add('loadBalancer.defaultAction', 'cannot be combined with `hostHeaders` or `pathPatterns`',
        'A default action matches everything the other rules do not, so it takes no conditions.');
    }
    if (lb.defaultAction !== true && hosts.length === 0 && paths.length === 0) {
      issues.add('loadBalancer.hostHeaders', 'is required unless `defaultAction: true`',
        'Without a condition the listener has nothing to match on. Give a host header ' +
  ...
  if (as !== undefined) {
    if (!isObject(as)) {
      ...
      const hasPolicy =
        as.cpuTargetPercent !== undefined ||
        as.memoryTargetPercent !== undefined ||
        as.requestsPerTarget !== undefined;
      if (!hasPolicy) {
        issues.add('autoScaling', 'needs at least one scaling target',
          'Set `cpuTargetPercent`, `memoryTargetPercent` and/or `requestsPerTarget`. ' +
            'Without one, capacity would never change.');
      }
      if (as.requestsPerTarget !== undefined && !isObject(m.loadBalancer)) {
        issues.add('autoScaling.requestsPerTarget', 'requires a load balancer',
          'Request-count scaling reads ALB metrics. Use CPU or memory targets for a service with no ALB.');
      }
    }
  }
}

function validateScheduledTasks(issues: IssueCollector, m: Record<string, any>): void {
  if (!Array.isArray(m.tasks) || m.tasks.length === 0) {
    issues.add('tasks', 'must list at least one scheduled job',
      'e.g. `- { name: nightly-sync, schedule: "cron(0 6 * * ? *)" }`');
    return;
  }

  const seen = new Set<string>();
  m.tasks.forEach((task: any, i: number) => {
    const at = `tasks[${i}]`;
    if (!isObject(task)) {
    ...
    if (typeof task.name !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(task.name)) {
      issues.add(`${at}.name`, 'must be ≤64 characters of letters, digits, dots, hyphens and underscores',
        'It becomes part of the EventBridge rule name.');
    } else if (seen.has(task.name)) {
      issues.add(`${at}.name`, `duplicates an earlier task named "${task.name}"`,
        'Rule names must be unique within the stack.');
    } else {
      seen.add(task.name);
    }

    if (typeof task.schedule !== 'string' || !SCHEDULE_PATTERN.test(task.schedule)) {
      issues.add(`${at}.schedule`, 'must be a `cron(...)` or `rate(...)` expression',
        'e.g. `cron(0 6 * * ? *)` for 06:00 UTC daily, or `rate(1 hour)`. ' +
...
function validateHealthCheck(issues: IssueCollector, hc: unknown): void {
  if (hc === undefined) return;
  if (!isObject(hc)) {
    issues.add('loadBalancer.healthCheck', 'must be an object');
    return;
  }
  const h = hc as Record<string, any>;
  ...
  if (typeof interval === 'number' && typeof timeout === 'number' && timeout >= interval) {
    issues.add('loadBalancer.healthCheck.timeoutSeconds', `(${timeout}s) must be less than intervalSeconds (${interval}s)`,
      'The ALB rejects a timeout that is not shorter than the interval.');
  }
...
function validateTaskSize(issues: IssueCollector, cpu: unknown, memory: unknown, at: string): void {
  if (typeof cpu !== 'number') {
    issues.add(`${at}.cpu`, 'is required and must be a number',
      `Valid Fargate values: ${Object.keys(FARGATE_SIZES).join(', ')} (1024 = 1 vCPU).`);
    return;
  }
  const size = FARGATE_SIZES[cpu];
  if (!size) {
    issues.add(`${at}.cpu`, `${cpu} is not a valid Fargate CPU value`,
      `Valid values: ${Object.keys(FARGATE_SIZES).join(', ')} (1024 = 1 vCPU).`);
    return;
  }
  if (typeof memory !== 'number') {
    issues.add(`${at}.memory`, 'is required and must be a number in MiB',
      `With cpu ${cpu}, memory must be ${size.min}–${size.max} MiB in ${size.step} MiB steps.`);
    return;
  }
  if (memory < size.min || memory > size.max || (memory - size.min) % size.step !== 0) {
    issues.add(`${at}.memory`, `${memory} MiB is not valid with cpu ${cpu}`,
      `With cpu ${cpu}, memory must be ${size.min}–${size.max} MiB in ${size.step} MiB steps.`);
  }
}

function validateSecrets(issues: IssueCollector, secrets: unknown): void {
  ...
  for (const [key, value] of Object.entries(secrets)) {
    ...
    if (!value.startsWith('arn:') || (!isSecretsManager && !isSsm)) {
      issues.add(`task.secrets.${key}`, 'must be a Secrets Manager or SSM Parameter Store ARN',
        'e.g. `arn:aws:secretsmanager:us-east-1:111122223333:secret:prod/db-AbCdEf:password::` ' +
...
function requireString(issues: IssueCollector, obj: Record<string, any>, field: string, path = field): void {
  if (typeof obj[field] !== 'string' || obj[field].length === 0) {
    issues.add(path, 'is required and must be a non-empty string');
  }
}

function optionalString(issues: IssueCollector, obj: Record<string, any>, field: string, path = field): void {
  if (obj[field] !== undefined && typeof obj[field] !== 'string') {
    issues.add(path, 'must be a string');
  }
}

function optionalBoolean(issues: IssueCollector, obj: Record<string, any>, field: string, path = field): void {
  if (obj[field] !== undefined && typeof obj[field] !== 'boolean') {
    issues.add(path, 'must be true or false');
  }
}

function optionalPort(issues: IssueCollector, obj: Record<string, any>, field: string, path = field): void {
...
function optionalStringArray(issues: IssueCollector, obj: Record<string, any>, field: string, path = field): void {
  const value = obj[field];
  if (value === undefined) return;
  if (!Array.isArray(value) || value.some((v) => typeof v !== 'string')) {
    issues.add(path, 'must be a list of strings',
      'Use argv form: `["node", "dist/job.js"]`, not a single shell string.');
  }
}

function optionalStringMap(issues: IssueCollector, obj: Record<string, any>, field: string, path = field): void {
  const value = obj[field];
  if (value === undefined) return;
  if (!isObject(value)) {
  ...
  for (const [k, v] of Object.entries(value)) {
    if (typeof v !== 'string') {
      issues.add(`${path}.${k}`, `must be a string, got ${typeof v}`,
